...
# sudoチェック
if [ "$EUID" -ne 0 ]; then
    print_color "31" "Please run as root or with sudo privileges"
    exit 1
fi
...
nvim +PlugInstall +qall || { print_color "31" "Failed to install Neovim plugins"; exit 1; }
print_color "32" "Installed Neovim plugins"

# Coc 拡張機能のインストール
if command -v node &> /dev/null; then
    print_color "36" "Installing Coc extensions..."
    nvim +CocInstall coc-json coc-tsserver coc-pyright coc-html coc-css +qall || { print_color "31" "Failed to install Coc extensions"; exit 1; }
